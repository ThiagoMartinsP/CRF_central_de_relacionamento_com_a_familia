...
Companion técnico do `README.md`. Aqui ficam as decisões de implementação, as
traduções de schema, o mecanismo de expiração por dentro e as arestas
conhecidas. O README explica o que o sistema faz; este documento explica como.
...
docs/
  especificação do recorte + estas notas
```
...
6. **Validação do DV do CPF rejeita com 422**, conforme seção 7 (validação na
   ...
   com DV válido.
7. **Templates a mais.** A spec descreve o comportamento sem nomear o template:
   `M1_PEDE_NOME_PRIMEIRO`, `M1_PEDE_NOME_SEGUNDO`, `ERRO_NOME_VAZIO`,
   `ERRO_TELEFONE_DUPLICADO` (obrigatório — a constraint
   `UNIQUE (cpf_crianca, telefone_e164)` existe e precisa de resposta ao
   usuário), `ERRO_LIMITE_CONTATOS` (defensivo, para o trigger), e os quatro do
   ciclo de expiração.
8. **`GET /criancas/{cpf}` devolve também o bloco `captura`** (sessão ativa com
   etapa e dados parciais, histórico de sessões, e se a criança está na fila). É
   endpoint de depuração; a spec pede criança + responsável + contatos.
...
10. **A reabertura por mensagem tardia encosta em autoatendimento**, que a seção
    ...
    iniciar nada novo.
11. **Os canônicos de `grau_relacao` são grafados com acento** (`avó`, `mãe`,
    `irmão`). O valor vai para o banco e é exibido de volta; `avo` no lugar de
    `avó` parece erro de digitação do sistema. O casamento sem acento é um
    segundo passe e só vale quando o resultado é único: `vovo` fica gravado como
    veio, porque pode ser avó ou avô, e chutar o gênero de uma pessoa da rede de
    apoio é pior do que guardar o texto cru.
